...
$linecolor = "#ccc";
$onclickevent = "null";
$stmt = $pdo->prepare('SELECT Title, SUM(Requests) as totals FROM ' . JOURNALTABLE . ' WHERE Inst_abbrev = :instabbrev AND product_abbrev = :prodabb AND start_date >= :startdate and start_date <= :enddate group by Title');
...
if ($results) {
	...
	for ($i = $keyarraygrp[0]; $i <= $maxnum; $i++) {
		...
		if (!array_key_exists($i, $chartarraygrp[0])) {
			$chartarraygrpexpanded[$_GET['inst']][$i] = 0;
		}
		else {
			$chartarraygrpexpanded[$_GET['inst']][$i] = $chartarraygrp[0][$i];
		}
	}
	...
	$chartarraygrpexpanded[$_GET['inst']] = $chartarraygrpexpanded[$_GET['inst']] + $outliers;
	...
	foreach($chartarraygrpexpanded[$_GET['inst']] as $grpnm => $grpexp) {
	...
	$stmtdate = $pdo->prepare('SELECT start_date, SUM(Requests) as totals FROM ' . JOURNALTABLE . ' WHERE Inst_abbrev = :instabbrev AND product_abbrev = :prodabb AND start_date >= :startdate and start_date <= :enddate group by start_date ORDER BY start_date asc');
