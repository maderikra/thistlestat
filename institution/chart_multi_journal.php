...
foreach($bencharray as $thisbench) {
	...
	if (isset($_GET['product'])) {
		...
		if ($results) {
			...
			$labelsarrdate = array();
			$totalsarrdate = array();
			$stmt = $pdo->prepare('SELECT start_date, SUM(Requests) as totals FROM  ' . JOURNALTABLE . '  WHERE Inst_abbrev = :instabbrev AND product_abbrev = :prodabb AND start_date >= :startdate and start_date <= :enddate group by start_date');
			$stmt->execute(['instabbrev' => $instnamecur, 'prodabb' => $productname, 'startdate' => $startdate, 'enddate' => $enddate]);
...
// grab first element in chart array, we don't know what the key (instname) is but who cares
...
foreach($chartarraygrp as $chartinst => $chartarr) {
	...
	// count total outliers in this inst's array (each inst's array should have the same number)

	$numoutliers = count($outliers[$chartinst]);
}
